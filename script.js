...
//       function displayAutocompleteResults(items) {
//           if (items && items.length > 0) {
//               const autocompleteSuggestions = items.map(item => item.volumeInfo.title);
//               autocompleteResults.innerHTML = autocompleteSuggestions.map(suggestion => `<div>${suggestion}</div>`).join('');
//           } else {
//               autocompleteResults.innerHTML = '';
//           }
//       }
